  ...
  <div class="min-h-screen bg-gradient-to-br from-blue-50 to-indigo-100 py-8">
    <div class="container mx-auto px-4">
      ...
      <div v-else-if="booking" class="max-w-5xl mx-auto">
        ...
        <div v-if="booking.canPay && booking.extraPay > 0" class="bg-orange-50 border-2 border-orange-200 rounded-2xl p-6 mb-6">
        ...
        <!-- Update Button -->
        <div v-if="booking.canUpdate" class="bg-blue-50 border-2 border-blue-200 rounded-2xl p-6 mb-6">
          <div class="flex flex-col md:flex-row items-center justify-between gap-4">
            <div class="flex items-center gap-4">
        ...
        <div class="bg-white rounded-2xl shadow-2xl max-w-md w-full mx-4 overflow-hidden">
            ...
            <div class="bg-yellow-50 border border-yellow-200 rounded-lg p-4">
              <div class="flex gap-2">
                ...
                <div class="text-sm text-yellow-800">
                  ...
                  <p>Cancelling this booking may affect refund eligibility.</p>
                </div>
              </div>
            </div>
          </div>
          ...
          <div class="bg-gray-50 px-6 py-4 flex gap-3 justify-end">
            ...
            >
              ...
              <span>{{ processingAction ? 'Cancelling...' : 'Yes, Cancel' }}</span>
            </button>
          </div>
        </div>
      </div>
...
const handlePay = async () => {
  ...
  const confirmed = confirm(
    `Confirm payment for booking ${booking.value.bookingID}?\n\n` +
    `Amount: Rp ${(booking.value.extraPay || booking.value.totalPrice).toLocaleString('id-ID')}`
  )
  ...
  try {
    ...
    await bookingStore.payBooking(booking.value.bookingID)
...
const handleRefund = async () => {
  ...
  const confirmed = confirm(
    `Request refund for booking ${booking.value.bookingID}?\n\n` +
    `Refund amount: Rp ${booking.value.refund.toLocaleString('id-ID')}`
  )
  ...
  try {
    processingAction.value = true
    await bookingStore.requestRefund(booking.value.bookingID)
    await fetchBookingDetail()
  } catch (error) {
